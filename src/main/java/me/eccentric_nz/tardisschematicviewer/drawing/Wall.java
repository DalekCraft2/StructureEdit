    ...
    public static void draw(GL4bc gl, Color color, float scale, float thickness, float sizeX, float sizeY, float sizeZ, Object properties) {
        ...
        if (properties instanceof String) {
            if (((String) properties).contains("up=false")) {
                ...
                Cube.draw(gl, color, scale, thickness, sizeY * 0.8f, thickness);
                gl.glTranslatef(0.0f, scale - sizeY * 0.8f, 0.0f);
            } else {
                Cube.draw(gl, color, scale, thickness, scale, thickness);
            }
            if (((String) properties).contains("south=low")) {
                ...
                Cube.draw(gl, color, scale, thickness, scale, (sizeZ - thickness) / 2.0f);
                gl.glTranslatef(0.0f, 0.0f, -thickness * 1.5f);
            }
            if (((String) properties).contains("east=low")) {
                ...
                Cube.draw(gl, color, scale, (sizeX - thickness) / 2.0f, scale, thickness);
                gl.glTranslatef(-thickness * 1.5f, 0.0f, 0.0f);
            }
